...
void dfs(int root) {
	for (int i = 0; i < graph[root].size(); i++) {
		...
		if (graph[graph[root][i]].size() == 0) vertex_inf[graph[root][i]].max_weight = sum_weight;
		// 자식의 max 가중치가 더 클 때
		if (vertex_inf[graph[root][i]].max_weight > vertex_inf[root].max_weight) vertex_inf[root].max_weight = vertex_inf[graph[root][i]].max_weight;
